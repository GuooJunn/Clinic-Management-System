...
def add_patient_records_staff():
    if request.method == 'POST':
        ...
        cur.execute("INSERT INTO PATIENT_RECORD (patient_name, patient_age, patient_gender, patient_blood_type, patient_medical_condition, patient_date_of_enrollment, patient_date_of_last_update, patient_covid_vacc_status, patient_allergies) VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s)", 
                    (name_input, age_input, gender_input, blood_type_input, medical_condition_input, today_date, today_date, covid_vacc_status_input, allergies_input))
        # Commit the transaction
        mysql.connection.commit()
        # Close the cursor
        cur.close()
...
def user_management_admin():
    if session['role'] == 'admin':
        ...
        return render_template('user-management-admin.html', user_data=data)
    else:
        return render_template('error403.html')
...
def add_user_management_admin():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        # Execute INSERT statement to add user data
        cur.execute("INSERT INTO users(user_name, user_email, user_password, user_contact_number, user_role) VALUE (%s, %s, %s, %s, %s)", 
